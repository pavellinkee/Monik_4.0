...
class TradingConfig(ConfigSection):
    ...
    #: Сколько находок проверять реальным предложением за один проход.
    #:
    #: Находка с завышенной котировкой выглядит лучшей и встаёт первой,
    #: заслоняя собой честные, — поэтому после отказа перебор идёт
    #: дальше. Но отказ стоит запросов, и в цикле, где ни одна находка не
    #: подтверждается, тратить их на весь список незачем.
    max_offer_checks: int = Field(default=10, ge=1, le=100)
